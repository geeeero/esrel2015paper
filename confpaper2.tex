...
we will now assume a range $[\yzl, \yzu]$ and a range $[\nzl, \nzu]$ of pseudocounts,
i.e., we now consider the set of priors
$\MZ := \{ p(\lambda\mid\nz,\yz) \mid \nz \in [\nzl, \nzu], \yz \in [\yzl, \yzu] \}$ 
as expressing our prior knowledge about component lifetimes.
...
technically, this leads to the set of posteriors being larger
when $\taut/n \not\in [\yzl, \yzu]$,
and thus to more cautious inferences.
...
***while so far only observed failures, here now also with censored observations.
