...
(Our set of posteriors for the Weibull scale parameter is thus like in Figure~\ref{fig:setofpost-nopdc}.)
The prior bounds $\lRsys(t)$ and $\uRsys(t)$ (red),
and the resulting posterior bounds $\lRsys(t \mid \vec{t}_2^3)$ and $\uRsys(t \mid \vec{t}_2^3)$ (blue),  
are depicted in Figure~***.
This tells us that, e.g., $P(T_\text{sys} > 20 \mid \vec{t}_2^3) = [***, ***]$.
\end{example}
