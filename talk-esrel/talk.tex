...
\item[\play]<3-> cautious assumptions on component reliability:
 \begin{itemize}
 \item[] expert information,\\ e.g. from the component manufacturer\\ \alert{which we don't trust entirely}
 \end{itemize}
\end{enumerate}
...
\uncover<4->{%
\node[cyanrand] (yl) at (1.5,-2.4) {$\yell = \E[\lambda\mid\mbf{t}]$};
...
\uncover<3-3>{\node at (3,2.5) {\parbox{0.7\textwidth}{\color{tuecyan} $\yfun{2} = \E[\lambda\mid\mbf{t}] = 53$ (6.4 weeks), $\sd = 61$\\ $\nfun{2} = 4$}};}
%
...
\uncover<6->{\node at (3,2.5) {\parbox{0.7\textwidth}{\color{tuecyan} $\yfun{2} = \E[\lambda\mid\mbf{t}] = 52$ (6.4 weeks), $\sd = 61$\\ $\nfun{2} = 4$}};}
...
\uncover<3-3>{\node at (3.5,0.1) {\parbox{0.7\textwidth}{\color{tuecyan}\small $\yfun{2} = \E[\lambda\mid\mbf{t}] \in [44, 70]$ (5.9--7.4 weeks),\\
...
\uncover<6-6>{\node at (3.9,0.1) {\parbox{0.7\textwidth}{\color{tuecyan}\small $\yfun{2} = \E[\lambda\mid\mbf{t}] \in [42, 92]$ (5.7--8.5 weeks),\\
...
\uncover<1->{%
\play\ Closed form for the system reliability:
\begin{multline*}
...
\uncover<1-1>{\node at (2.9, 2.3) {\parbox{0.5\textwidth}{\color{tuered}\small $\nz_1 \in [2, 10]$, $\E[T\mid\yz_1] \in [9, 11]$\\
                                                                             $\nz_2 \in [2, 10]$, $\E[T\mid\yz_2] \in [4,\phantom{1}5]$\\
                                                                             $\nz_3 \in [1,\phantom{1}5]$, $\E[T\mid\yz_3] \in [9, 11]$}};}
...
%\uncover<3-3>{\node at (0,0) {\includegraphics[width=0.95\textwidth]{sysex1-3}};}
%\uncover<3-4>{\node at (3.5, 0.5) {\parbox{0.25\textwidth}{\color{tuepms137}\small $\mbf{t} = (1,2,2^+)$}};}
%\uncover<4-4>{\node at (0,0) {\includegraphics[width=0.95\textwidth]{sysex1-4}};}
%\uncover<4-4>{\node at (3.5,-0.5) {\parbox{0.25\textwidth}{\color{tuepmsgreen}\small $\mbf{t} = (15^+,15^+,15^+)$}};}
\fill (0,0) circle (3pt);
...
\begin{itemize}
\item[\play]<1-> Very limited data: Bayesian model with set of conjugate priors %for component reliability
\item[\play]<2-> Set of system reliability functions %appropriately
reflects uncertainties from limited data (with censoring!) and vague expert information  
\item[\play]<3-> In particular, it reflects \emph{prior-data conflict} %between the two sources of information 
%Outlook
...
\begin{itemize}
\item[\play]<5-> Nonparametric model\\ (drop Weibull assumption for component lifetimes)
\item[\play]<6-> Allow dependence between components\\ (common-cause failure, \ldots) %or more explicit dependence
\item[\play]<7-> Use model for maintenance planning %, design
\end{itemize}}};
\end{tikzpicture}
}
