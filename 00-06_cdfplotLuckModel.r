...
setMethod("cdfplot", "LuckModel", function(object, xvec = 100, epsilon = 1e-5, #
 ...
 for (i in 1:.xveclength){
  .yvec[i] <- wrapOptim(par = c((.n0l+.n0u)/2, (.y0l+.y0u)/2),  # initial value for .n0y0: centre of the prior set
  ...
  .yvecrev[i] <- wrapOptim(par = c((.n0l+.n0u)/2, (.y0l+.y0u)/2),  # initial value for .n0y0: centre of the prior set
